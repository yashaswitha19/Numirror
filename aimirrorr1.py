...
# Set page config
st.set_page_config(
    page_title="Fashion & Mood Assistant",
    page_icon="👗",
    layout="wide",
...
def color_rgb_to_name(rgb_color):
    """Convert RGB color to a readable color name"""
    if not rgb_color:
        return "unknown"
...
def main():
    st.title("👗🎭 Fashion & Mood Assistant")
    st.markdown("---")
    ...
    with col1_sidebar:
        if st.button("👗 Fashion Mode", use_container_width=True):
    ...
    with col2_sidebar:
        if st.button("🎭 Emotion Mode", use_container_width=True):
            stop_current_speech() # Stop any ongoing speech
    ...
    with col2:
        ...
        if st.session_state.current_mode and (st.session_state.last_outfit_description or st.session_state.last_emotion):
                ...
                if user_speech:
                    ...
                    if any(word in user_speech for word in ["quit", "exit", "bye", "goodbye"]):
                        ...
                        with st.spinner("🤖 Generating response..."):
                            ...
                            else: # This is the original, unchanged logic for emotion mode
